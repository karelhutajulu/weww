...
void stu_bitwise(std::span<std::int8_t> result, std::span<const std::int8_t> a,
                 std::span<const std::int8_t> b) {
    constexpr std::uint64_t kMaskA5 = 0xA5A5A5A5A5A5A5A5ull;
    constexpr std::uint64_t kMask99 = 0x9999999999999999ull;

    const std::size_t n = std::min({result.size(), a.size(), b.size()});
    const auto *src_a = reinterpret_cast<const std::uint8_t *>(a.data());
    const auto *src_b = reinterpret_cast<const std::uint8_t *>(b.data());
    ...
    for (; i + 8 <= n; i += 8) {
        std::uint64_t va;
        std::uint64_t vb;
        std::memcpy(&va, src_a + i, sizeof(va));
        std::memcpy(&vb, src_b + i, sizeof(vb));
        const std::uint64_t out = kMaskA5 ^ ((va | vb) & kMask99);
        std::memcpy(dst + i, &out, sizeof(out));
    }

    for (; i < n; ++i) {
        dst[i] = static_cast<std::uint8_t>(0xA5u ^ ((src_a[i] | src_b[i]) & 0x99u));
    }
}
