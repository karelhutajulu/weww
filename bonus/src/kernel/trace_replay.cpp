...
void stu_trace_replay_wrapper(void* ctx) {
    ...
    uint64_t total = 0;
    constexpr uint64_t order_mix = 1315423911ull;
    const uint64_t *costs = args.record_costs.data();
    const uint32_t *trace_data = args.trace.data();
    const size_t n = args.trace.size();

    for (size_t i = 0; i < n; ++i) {
        if (i + 32 < n) {
            __builtin_prefetch(costs + trace_data[i + 32], 0, 1);
        }
        total = total * order_mix + costs[trace_data[i]];
    }
    args.out = total;
}
