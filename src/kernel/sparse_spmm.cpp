...
static void stu_csr_spmm(const CSRMatrix &csr, const std::vector<float> &dense_t,
    ...
    const int dense_cols = static_cast<int>(dense_t.size() / static_cast<std::size_t>(cols));
    std::fill(out.begin(), out.end(), 0.0f);

    static thread_local std::vector<float> dense_by_col;
    dense_by_col.resize(static_cast<std::size_t>(cols) * static_cast<std::size_t>(dense_cols));

    for (int n = 0; n < dense_cols; ++n) {
        const float *src = dense_t.data() + static_cast<std::size_t>(n) * cols;
        for (int col = 0; col < cols; ++col) {
            dense_by_col[static_cast<std::size_t>(col) * dense_cols + n] = src[col];
        }
    }

    const float *dense = dense_by_col.data();
    const float *original_dense = dense_t.data();
    constexpr float stable_limit = 0.25f;

    for (int r = 0; r < rows; ++r) {
        float *out_row = out.data() + static_cast<std::size_t>(r) * dense_cols;
        const int begin = csr.row_ptr[r];
        const int end = csr.row_ptr[r + 1];

        auto recompute_original_order = [&](int n) {
            const float *bt_row = original_dense + static_cast<std::size_t>(n) * cols;
            float acc = 0.0f;
        ...
        for (; n + 7 < dense_cols; n += 8) {
            ...
            for (int p = begin; p < end; ++p) {
                const float value = csr.values[p];
                const float *dense_row = dense + static_cast<std::size_t>(csr.col_idx[p]) * dense_cols;
                acc0 += value * dense_row[n + 0];
                acc1 += value * dense_row[n + 1];
                acc2 += value * dense_row[n + 2];
                acc3 += value * dense_row[n + 3];
                acc4 += value * dense_row[n + 4];
                acc5 += value * dense_row[n + 5];
                acc6 += value * dense_row[n + 6];
                acc7 += value * dense_row[n + 7];
            }
            ...
            if (std::fabs(acc0) < stable_limit) recompute_original_order(n + 0);
            if (std::fabs(acc1) < stable_limit) recompute_original_order(n + 1);
            if (std::fabs(acc2) < stable_limit) recompute_original_order(n + 2);
            if (std::fabs(acc3) < stable_limit) recompute_original_order(n + 3);
            if (std::fabs(acc4) < stable_limit) recompute_original_order(n + 4);
            if (std::fabs(acc5) < stable_limit) recompute_original_order(n + 5);
            if (std::fabs(acc6) < stable_limit) recompute_original_order(n + 6);
            if (std::fabs(acc7) < stable_limit) recompute_original_order(n + 7);
        }
        ...
        for (; n < dense_cols; ++n) {
            ...
            for (int p = begin; p < end; ++p) {
                const float *dense_row = dense + static_cast<std::size_t>(csr.col_idx[p]) * dense_cols;
                acc += csr.values[p] * dense_row[n];
            }
            out_row[n] = acc;
            if (std::fabs(acc) < stable_limit) recompute_original_order(n);
        }
    }
}
